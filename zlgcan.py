...
_LIB_NAME = f"./{_PREFIX}zlgcan_driver_rs_api.{platform.machine()}.{_EXTEND}"
...
_LIB.zlgcan_chl_cfg.restype = c_void_p
...
class ZCanBus(can.BusABC):
    ...
    def __init__(self,
        ...
        for idx, cfg in enumerate(configs):
            ...
            cfg_ptr = _LIB.zlgcan_chl_cfg(factory, _cfg, byref(error))
            if not cfg_ptr:
                raise can.CanOperationError(error.value.decode("utf-8"))
